...
OC3:        IN      ACIA_CTRL ;COME HERE TO DO OUTPUT
            ANI     2H ;STATUS BIT
            JZ      OC3 ;NOT READY, WAIT
            POP     PSW ;READY, GET OLD A BACK
            OUT     ACIA_DATA ;AND SEND IT OUT
            CPI     CR ;WAS IT CR?
            RNZ      ;NO, FINISHED
...
; the problem you are mention is caused by terminal. My terminal sends 08h for backspace and 1Bh for Escape. TINY BASIC takes 7Fh for backspace (RUB-OUT) and 7Dh for delete line.

;It is possible to change those constants at lines 1113 and 1120 (from 7Fh to 08h, from 7Dh to 1Bh), or update the CHKIO routine in the way I attached below.

;Caution! When you press the backspace key, TINY BASIC enter the backslash character. It is not a bug, it is a desired behavior (I guess it's for old printer terminals / teletype, which can not "delete a character"). So you will end with:
...
CHKIO:      IN      ACIA_CTRL ;*** CHKIO ***
            ...
            IN      ACIA_DATA ;READY, READ DATA
            ANI     7FH ;MASK BIT 7 OFF
            CPI     08H ;MM: IS IT A BACKSPACE?
            JZ      CIRUB 
            CPI     1BH ;MM: IS IT AN ESCAPE?
            JZ      CIESC 
            CPI     0FH ;IS IT CONTROL-O?
            JNZ     CI1 ;NO, MORE CHECKING
            LDA     OCSW ;CONTROL-O FLIPS OCSW
...
CI1:        CPI     3H ;IS IT CONTROL-C?
            RNZ      ;NO, RETURN "NZ"
            JMP     RSTART ;YES, RESTART TBI
CIRUB:      MVI     A,7Fh ;RUBOUT
            JMP     CI1 
CIESC:      MVI     A,7Dh ;DELETE LINE
            JMP     CI1 
MSG1:       DB      "TINY " 
            DB      "BASIC" 
            DB      CR 
